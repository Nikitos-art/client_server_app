...
class ClientMainWindow(QMainWindow):
    def __init__(self, database, transport):
        ...
        # Loading window configs from design
        ...
        self.current_chat = None  # Current contact with whom message exchange is taking place
        self.ui.list_messages.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.ui.list_messages.setWordWrap(True)

        # Double click on contact list is sent to the processor
    ...
    # Deactivate entry field
    def set_disabled_input(self):
        # Signature  - sender.
        self.ui.label_new_message.setText('For choosing a sender '
                                          'double click on it in contacts window.')
        self.ui.text_message.clear()
        if self.history_model:
            self.history_model.clear()

        # Field entry and button for sending are inactive before receivers not inactive
        self.ui.btn_clear.setDisabled(True)
        self.ui.btn_send.setDisabled(True)
        self.ui.text_message.setDisabled(True)

    # Filing in message history.
    def history_list_update(self):
        # Receiving history sorted by date
        list_messages = sorted(self.database.get_history(self.current_chat),
                               key=lambda item: item[3])
        # IF model is not created then create it.
        if not self.history_model:
            self.history_model = QStandardItemModel()
            self.ui.list_messages.setModel(self.history_model)
        # Clear old entries
        self.history_model.clear()
        # Take first no more than 20 last entries.
        length = len(list_messages)
        start_index = 0
        if length > 20:
            start_index = length - 20
        # FIll in model entry, also separate inbox and outbox
